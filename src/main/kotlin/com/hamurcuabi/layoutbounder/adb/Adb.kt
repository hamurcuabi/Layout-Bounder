...
import com.hamurcuabi.layoutbounder.LOG
...
import com.hamurcuabi.layoutbounder.model.Device.ConnectionType.USB
import com.hamurcuabi.layoutbounder.model.Device.ConnectionType.WIFI
...
import com.hamurcuabi.layoutbounder.utils.findAdbExecInSystemPath
...
class Adb(
    ...
    fun toggleLayoutBound(state: String, deviceId: String): Flow<LogEntry> = flow {
        ...
        "-s $deviceId shell setprop debug.layout $currentState".execAndLog(this)
        "-s $deviceId shell service call activity 1599295570".execAndLog(this)
    }

    fun getLayoutBound(deviceId: String): String {
        return "-s $deviceId shell getprop debug.layout".exec().firstLine().trim()
    }
